...
auto grad_output = torch::ones({1}).expand({_n_out, _n_in}).to(device);
auto gradient = torch::autograd::grad(  {output},
                       {single_input},
...
std::cout << "- Output: \t" << out_vec << std::endl;
std::cout << "- Gradients: \t" << der << std::endl;
